...
namespace WrapGui
{
	...
	int GuiApp::Internal_Init()
	{
		...
		VkResult err = glfwCreateWindowSurface(g_Instance, m_MainWindow, g_Allocator, &surface);
		check_vk_result(err);

		// Create Framebuffers
		int w, h;
		glfwGetFramebufferSize(m_MainWindow, &w, &h);
		...
		ImFont* robotoFont = io.Fonts->AddFontFromMemoryCompressedTTF(Font_compressed_data, Font_compressed_size, 18.0f, &fontConfig);
	...
	void GuiApp::SetMain(const std::function<void()>& function)
	{
		m_Function = function;
	}
